...
class FakePlayer:
    ...
    def fake_move(self, x1, y1, x2, y2):
        self.map[x1, y1] = 0
        self.map[x2, y2] = 1
...
def main():
    try:
        ...
        player.fake_move(4,9,4,8)
        player.move(5, 0, 6, 0)
        raw_input("enter to go")
        player.fake_move(0,2,0,0)
        player.move(6, 0, 0, 0)
        raw_input("enter to go")
        player.fake_move(5,2,4,2)
        player.move(7,0,7,9)
        ...
        #     if s.startswith("r"):
        #         cstr = s.split('-')[1]
        #         coord = map(int, cstr.split(','))
        #         player.map[coord[0], coord[1]] = 0
        #     elif s.startswith("m"):
        #         cstr = s.split('-')[1]
        #         coord = map(int, cstr.split(','))
        #         player.map[coord[0], coord[1]] = 0
        #         player.map[coord[2], coord[3]] = 1
        #     else:
        #         raw_input("Press ENTER to excute action: {}".format(s))
        #         player.parse_code(s)
