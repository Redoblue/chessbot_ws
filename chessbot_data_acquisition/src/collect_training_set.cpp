...
private:
  ...
  bool save, startRecording, stopRecording;
  bool running;
  size_t frame, tracklet;
...
public:
  Receiver(const std::string &topicColor, const std::string &topicDepth, const bool useExact, const bool useCompressed)
    : topicColor(topicColor), topicDepth(topicDepth), useExact(useExact), useCompressed(useCompressed),
      updateImage(false), updateCloud(false), save(false), startRecording(false), stopRecording(false), 
      running(false), frame(0), tracklet(0), queueSize(5), nh("~"), spinner(0), it(nh), mode(CLOUD)
  {
...
private:
  ...
  {
    oss.str("");
    oss << "./" << std::setfill('0') << std::setw(4) << tracklet;
    const std::string baseName_T = oss.str();
    oss.str("");
    oss << std::setfill('0') << std::setw(4) << frame;
    const std::string baseName_F = oss.str();
    const std::string cloudName = baseName_T + "_" + baseName_F + "_cloud.pcd";
    const std::string colorName = baseName_T + "_" + baseName_F + "_color.jpg";
    const std::string depthName = baseName_T + "_" + baseName_F + "_depth.png";
    const std::string depthColoredName = baseName_T + "_" + baseName_F + "_depth_colored.png";

    // 在采集训练集的时候，我们将跳过点云数据和深度着色图
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "collect_training_set", ros::init_options::AnonymousName);
