...
private:
  ...
  void imageViewer()
  {
    ...
    cv::Point* mpos = new cv::Point(0, 0);

    cv::namedWindow("Image Viewer");
    cv::setMouseCallback("Image Viewer", onMouseEvent, mpos);
    oss << "starting...";
    ...
    for(; running && ros::ok();)
    {
      if(updateImage)
      {
        ...
        /* 点云坐标 */
        float px, py, pz;
        queryCloudCoord(depth, mpos->x, mpos->y, px, py, pz);
        oss.str("");
        oss << "r:" << mpos->y << ", c:" << mpos->x << ", x:" << px << ", y:" << py << ", z:" << pz;
        cv::putText(combined, oss.str(), pos, font, sizeText, colorText, lineText, CV_AA);
        // cv::putText(combined, oss.str(), cv::Point(5, 40), font, sizeText, colorText, lineText, CV_AA);
  ...
  static void onMouseEvent(int event, int x, int y, int flags, void* pMPos)
  {
  ...
  void queryCloudCoord(const cv::Mat &depth, int &col, int &row, float &x, float &y, float &z) const
  {
    const float badPoint = std::numeric_limits<float>::quiet_NaN();
    const uint16_t *itD = depth.ptr<uint16_t>(row);
    const float ly = lookupY.at<float>(0, row);
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "lookup_coordinate", ros::init_options::AnonymousName);
